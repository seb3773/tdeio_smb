...
4. **SNMP Printer Model Identification (UDP 161)**: Automatically identifies network printers and issues fast `GetNextRequest` queries for `hrDeviceDescr` (`1.3.6.1.2.1.25.3.2.1.3`) and fallback `sysDescr` (`1.3.6.1.2.1.1.1`) across non-blocking UDP sockets in parallel (~300ms). Discovered printers display clean printer icons (`print/printer`) and their exact commercial model names (e.g. *Brother MFC-J6720DW*, *HP LaserJet 400 color M451nw*, *Canon MF650C Series*, *KONICA MINOLTA bizhub C251i*), with automatic disambiguation (`Model (HOST)`) when identical models exist on the network.
